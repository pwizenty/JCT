...
public class DomainDrivenDesignAnalyzer implements Analyzer {
    ...
    private Result checkEntityConstrains(Type type) {
        ...
        if (identity) {
            return new Result(type.getName(), 1, "Entity has an identifier", ResultStatus.CORRECT);
        } else {
            return new Result(type.getName(), 1, "Entity has no identifier", ResultStatus.VIOLATION);
        }
    }
